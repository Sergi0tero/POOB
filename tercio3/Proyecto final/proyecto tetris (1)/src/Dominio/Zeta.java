...
public class Zeta extends Figura{
        ...
        for(int i = 19; i>=0; i-=1) {
            for (int j = 9; j >= 0; j -= 1) {
                if(tablero[i][j] == 3  && i < 19 && i > 0){
                    ...
                    else if (j < 8 && tablero[i][j+1] == 6 || tablero[i-1][j+2] == 6 || tablero[i-2][j+2] == 6 || tablero[i][j+1] == 7 || tablero[i-1][j+2] == 7 || tablero[i-2][j+2] == 7 || tablero[i][j+1] == 8 || tablero[i-1][j+2] == 8 || tablero[i-2][j+2] == 8|| tablero[i][j+1] == 9 || tablero[i-1][j+2] == 9 || tablero[i-2][j+2] == 9) {
                        bufocapturado();
                    }
    ...
    public int[][] mueveIzquierda() {
        ...
        for(int i = 19; i>=0; i-=1) {
            for (int j = 9; j >= 0; j -= 1) {
                if(tablero[i][j] == 3  && i < 19 && j>3 && i > 0){
                    if(currentflag == 1 && i > 1) {
                    ...
                    }else if (tablero[i][j-1] == 6 || tablero[i-1][j-1] == 6 || tablero[i-2][j] == 6 || tablero[i][j-1] == 7 || tablero[i-1][j-1] == 7 || tablero[i-2][j] == 7 || tablero[i][j-1] == 8 || tablero[i-1][j-1] == 8 || tablero[i-2][j] == 8 || tablero[i][j-1] == 9 || tablero[i-1][j-1] == 9 || tablero[i-2][j] == 9) {
                        bufocapturado();
                    }
                }
                if (currentflag == 1 && i > 0 && j > 2 && tablero[i][j] == 3 && tablero[i - 1][j-2] == 3 && tablero[i][j - 2] >= 0 && tablero[i - 1][j - 3] >= 0 && tablero[i][j - 2] != -2 && tablero[i - 1][j - 3] != -2 && bandera) {
                    ...
                    tablero[i][j - 2] = 3;
                    tablero[i - 1][j - 3] = 3;
                } else if(currentflag == 2 && i > 2 && j < 8 && j>0 && tablero[i][j] == 3 && tablero[i - 2][j+1] == 3 && tablero[i][j - 1] >= 0 && tablero[i - 1][j - 1] >= 0 && tablero[i - 2][j] >= 0 && tablero[i][j - 1] != -2 && tablero[i - 1][j - 1] != -2 && tablero[i - 2][j] != -2 && bandera) {
                    bandera = false;
                    tablero[i][j] = 0;
                    tablero[i - 1][j + 1] = 0;
                    tablero[i - 2][j + 1] = 0;
