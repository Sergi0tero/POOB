...
public class World
{
    ...
    public World(int nation, int routes[][], int armies[][]){
        if (nation <= 5){
            ...
            actualizarNaciones();
            graph = new Graph(naciones, routes);
        }
    }
    
    private void actualizarNaciones(){
        this.naciones = new int[nations.size()];
        for (int i = 0; i < nations.size();i++){
            naciones[i] = i;
        }
    }
    
    // public void actualizarRoute(){
        // int[][] arcos = new int [routeList.size()][3];
        // int cont = 0;
        // for (int i = 0; i < routeList.size(); i++){
            // for (Route i: routeList.()){
                // arcos[cont][0] = indexNation(i.getLocation("locationA"));
                // arcos[cont][1] = indexNation(i.getLocation("locationB"));
                // arcos[cont][2] = i.getCost();
                // cont += 1;
            // }
        // }
    // }
    ...
    public void moveArmyOneRoute(String locationA, String locationB){
        ...
        if (routeList.containsKey(key)){
            if (nations.get(locationA).getArmy() > 0){
                if (nations.get(locationA).isConquered() && nations.get(locationB).isConquered()){
    ...
    /**
     * Intenta conquistar la nacion dada
     * @param nation String color de la nacion que se quiere conquistar
     */
    public void tryToConquer(String nation){
        HashMap hash = new HashMap();
        hash = graph.bellmanFord(1);
        for (int i = 0; i < hash.size(); i++){
            System.out.println(hash.get(i).get());
